...
const Navbar = () => {
  ...
  return (
    <div className="fixed top-0 left-0 w-full flex items-center justify-between py-4 px-6 font-medium bg-white z-50 shadow-sm">
      ...
      <ul className='hidden sm:flex gap-5 text-sm text-gray-700'>
        <NavLink to='/' className='flex flex-col items-center gap-1'>
            <p>HOME</p>
            <hr className='w-2/4 border-none h-[1.5px] bg-[#4D3B4C] hidden' />
        </NavLink>
        ...
        <div className='group relative'>
          ...
          <div className='group-hover:block hidden absolute dropdowm-menu right-0 pt-4'>
            <div className='flex flex-col gap-2 w-36 py-3 px-5 bg-slate-100 text-gray-500 rounded'>
              <p className='cursor-pointer hover:text-hoverPurple'>My Profile</p>
              <p className='cursor-pointer hover:text-hoverPurple'>Orders</p>
              <p className='cursor-pointer hover:text-hoverPurple'>Logout</p>
            </div>
          </div>
        </div>
    ...
    <div className={`fixed top-0 right-0 bottom-0 h-screen bg-white transition-all duration-300 ease-in-out z-[9998] ${visible ? 'w-full' : 'w-0 overflow-hidden'}`}>
